...
namespace objectsfm {
		...
		std::vector<double>* f, std::vector<Eigen::Vector3d>* depths)
	{
		...
		Eigen::Matrix<double, 36, 1> coeffs;
		coeffs(0) = 1.0;
		...
		Eigen::Matrix<double, 78, 78> coefficient_matrix;
		coefficient_matrix.setZero();
		Eigen::Matrix<double, 78, 10> b;
		b.setZero();
		...
		const Eigen::Matrix<double, 78, 10> solved_mat = coefficient_matrix.partialPivLu().solve(b);

		Eigen::Matrix<double, 10, 10> eigen_mat = Eigen::Matrix<double, 10, 10>::Zero();
		eigen_mat(0, 1) = 1.0;
		eigen_mat(1, 5) = 1.0;
		eigen_mat(2, 6) = 1.0;
		eigen_mat(3, 7) = 1.0;
		...
		Eigen::EigenSolver<Eigen::Matrix<double, 10, 10> > eigensolver(eigen_mat);
		const Eigen::EigenSolver<Eigen::Matrix<double, 10, 10> >::EigenvectorsType
		...
		for (int i = 0; i < 10; i++) 
		{
			if (sol(3, i).imag() != 0 || sol(3, i).real() < 0 || std::isnan(sol(3, i).real())) 
			{
				continue;
			}
