...
namespace objectsfm {
	...
	void SimilarityGraph::AssociateDatabase(Database * db)
	{
		db_ = db;
	}

	void SimilarityGraph::BuildSimilarityGraph(int method, std::vector<std::vector<float>> &similarity_matrix)
	{
		if (method == 1) {
			SimilarityGraphInvFile(similarity_matrix);
		}
		else {
			SimilarityGraphBowDistance(similarity_matrix);
		}
	}


	void SimilarityGraph::SimilarityGraphInvFile(std::vector<std::vector<float>> &similarity_matrix)
	{
	...
	void SimilarityGraph::GenerateInvertedFile(std::vector<std::vector<int>> &inverted_file)
	{
	...
	void SimilarityGraph::SimilarityGraphBowDistance(std::vector<std::vector<float>> &similarity_matrix)
	{
		num_imgs_ = db_->num_imgs_;
