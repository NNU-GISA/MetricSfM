...
class FBOW_API Vocabulary
{
 ...
 public:
    ...
    void transform(const cv::Mat &features, std::vector<std::vector<int>> &ids, std::vector<std::vector<float>> &dis)throw(std::exception);

	fBow transform(const cv::Mat &features, std::vector<int> &word_ids)throw(std::exception);
...
public:
    ...
    void _transform(const cv::Mat &features, std::vector<std::vector<int>> &ids, std::vector<std::vector<float>> &dis)throw(std::exception){
        Computer comp;
        ...
        fBow result;
		ids.resize(features.rows);
		dis.resize(features.rows);
        std::pair<DType, uint32_t> best_dist_idx(std::numeric_limits<uint32_t>::max(),0);  //minimum distance found
		std::pair<DType, uint32_t> seco_dist_idx(std::numeric_limits<uint32_t>::max(), 0); //second  distance found
		std::pair<DType, uint32_t> thir_dist_idx(std::numeric_limits<uint32_t>::max(), 0); //third   distance found
        block_node_info *bn_info_best, *bn_info_seco, *bn_info_thir;
        for(int cur_feature=0;cur_feature<features.rows;cur_feature++)
		{
			ids[cur_feature] = std::vector<int>(3, -1);
			dis[cur_feature] = std::vector<float>(3, -1.0);
            ...
            do{
				...
				seco_dist_idx.first = std::numeric_limits<uint32_t>::max();
				thir_dist_idx.first = std::numeric_limits<uint32_t>::max();
                ...
                for(int cur_node=0;cur_node<c_block.getN();cur_node++)
                {
					...
					else if (d<seco_dist_idx.first) seco_dist_idx = std::make_pair(d, cur_node);
					else if (d<thir_dist_idx.first) thir_dist_idx = std::make_pair(d, cur_node);
                }
				bn_info_best = c_block.getBlockNodeInfo(best_dist_idx.second);
				bn_info_seco = c_block.getBlockNodeInfo(seco_dist_idx.second);
				bn_info_thir = c_block.getBlockNodeInfo(thir_dist_idx.second);

				//if the node is leaf get word id and weight,else go to its children
				if (bn_info_seco->isleaf()) {
					ids[cur_feature][1] = bn_info_seco->getId();
					dis[cur_feature][1] = seco_dist_idx.first;
				}
				if (bn_info_thir->isleaf()) {
					ids[cur_feature][2] = bn_info_thir->getId();
					dis[cur_feature][2] = thir_dist_idx.first;
				}
				if (bn_info_best->isleaf()) {
					ids[cur_feature][0] = bn_info_best->getId();
					dis[cur_feature][0] = best_dist_idx.first;
				}
				else setBlock(bn_info_best->getId(), c_block);//go to its children
			} while (!bn_info_best->isleaf() && bn_info_best->getId() != 0);
        }
    }

	template<typename Computer>
	fBow _transform(const cv::Mat &features, std::vector<int> &word_ids)throw(std::exception) {
		Computer comp;
		comp.setParams(_params._desc_size, _params._desc_size_bytes_wp);
		using DType = typename Computer::DType;//distance type
		using TData = typename Computer::TData;//data type
		...
		fBow result;
		word_ids.resize(features.rows, -1);
		std::pair<DType, uint32_t> best_dist_idx(std::numeric_limits<uint32_t>::max(), 0);//minimum distance found
		...
		for (int cur_feature = 0; cur_feature<features.rows; cur_feature++) {
			...
			do {
				...
				if (bn_info->isleaf())
				{
					...
					word_ids[cur_feature] = bn_info->getId();
				}
